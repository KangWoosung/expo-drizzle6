...
const MigrationFallback = ({ error }: { error?: Error }) => {
  return (
    <View className="flex-1 bg-gray-100 justify-center items-center">
      <View className="bg-white rounded-lg shadow-lg p-6 w-3/4 max-w-md">
        {error ? (
          <View>
            <Text className="text-lg font-bold text-gray-800 text-left mb-4">
              데이터베이스 업데이트 중 에러가 발생했습니다.
            </Text>
            <Text className="text-sm text-gray-600 text-left mb-6">
              {error?.message}
            </Text>
          </View>
        ) : (
